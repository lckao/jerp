...
function getSelectionString(){//created by folier
    if(window.getSelection)
        return dialog.Selection.GetSelection().GetSelection();
    else if(document.selection && document.selection.createRange)
       return dialog.Selection.GetSelection().SelectionData.text;
    else
        return '';
}
...
{
	if ( !oLink ){
        GetE('showText').value = getSelectionString();
        return ;
    }

	var sType = 'url' ;
    GetE('showText').value=oLink.innerHTML;
...
function Ok()
{
	...
	for ( var i = 0 ; i < aLinks.length ; i++ )
	{
        ...
        if(GetE('showText').value==''){
            alert('请先填写链接显示文字.');
            GetE('showText').focus();
            return;
        }

		oLink.innerHTML = GetE('showText').value;// sInnerHtml;// Set (or restore) the innerHTML
...
function SetUrl( url, fileName )
{
	...
	loc=loc.substring(0,loc.indexOf("JDiy_bin/fck/editor"));
	url=loc+url;//*******************************************************by: folier

	GetE('txtUrl').value = url ;

    if(fileName==undefined) fileName =url.substring(url.lastIndexOf("/")+1); //***********by folier
    GetE('showText').value=fileName;//****************************by folier     todo
	OnUrlChange() ;
	dialog.SetSelectedTab( 'Info' ) ;
}


function zltUpSuccess(path, fileName){//******************************************by:folier
	window.parent.Throbber.Hide() ;
	GetE( 'divUpload' ).style.display  = '' ;
	SetUrl( path, fileName ) ;
	GetE('frmUpload').reset() ;
	parent.parent.addFile(path.replace(/.*JDiy_data\/editor\//i,""));
}
